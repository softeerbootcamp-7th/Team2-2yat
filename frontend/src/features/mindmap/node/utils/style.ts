...
export const colorBySize = (size: "sm" | "md" | "lg", color: NodeColor | undefined) => {
    const border = borderClass(color, 100);

    switch (size) {
        case "sm":
            return `border ${border} bg-white hover:border hover:${border} hover:bg-white focus:border-2 focus:${border} focus:shadow-md`;
        case "lg":
            return `border ${border} ${bgClass(color, 15)} hover:border hover:${border} hover:${bgClass(color, 15)} focus:border-2 focus:${bgClass(color, 15)} focus:shadow-md`;
        case "md":
        default:
            return `border ${border} ${bgClass(color, 5)} hover:border hover:${border} hover:${bgClass(color, 5)} focus:border-2 focus:${bgClass(color, 5)} focus:shadow-md`;
    }
};
